        ...
        for num_sims = 1:S.config.NUM_SIMS
            ...
            while (S.config.time(end) < S.config.tf) && ~(S.path.reach_end_mhempc)
                ...
                if flag; break; else
                    ...
                    tic;
                    S.exec_time.t_tot   = [S.exec_time.t_tot, [S.exec_time.t_mhe(end); S.exec_time.t_pfa(end);...
                        max([S.exec_time.t_mpc(end),S.exec_time.t_labc(end),S.exec_time.t_laobc(end),S.exec_time.t_rl1dim(end)]); S.exec_time.t_mis(end)]];
...
function S = build_setup(S)
    ...
    S.config.controller     = 'laobc'; % 'mpcCasadi', 'laobc', 'rl'
    ...
    S.config.tf             = 25*120;%1000;                                     % final time of sym. [Seg]
    ...
    S.config.w0             = 0;
    S.config.vf             = 0.01;%/S.config.Ts;%0.1;
    S.config.vl             = 0;
    S.config.vz             = 0;    
...
function S = callController(S)
    ...
    elseif strcmp(S.config.controller,'labsat')
        S = call_LABC_SAT(S);
    elseif strcmp(S.config.controller,'rl')
        S = call_RL1dim(S);
    else
...
function S = call_RL1dim(S)
    ...
    %
    v                       = -(S.controller.ref(1:2) - S.data.xest(1:2,end));
    theta                   = S.data.xest(4,end);
    % x-axe legged robot
    vx                      = [cos(theta); sin(theta)];    
    ref_x                   = norm(dot(v,vx)*vx)*sign(dot(v,vx));
    S.algorithms.RL.ex      = ref_x;
    S.algorithms.RL.exInt   = S.algorithms.RL.exInt + S.algorithms.RL.ex * S.config.Ts;
    state_x                 = [ref_x; S.algorithms.RL.vf; S.algorithms.RL.ex; S.algorithms.RL.exInt];
    action_x                = getAction(S.algorithms.RL.alg.agent, state_x);
    S.algorithms.RL.vf      = S.algorithms.RL.vf + action_x{1}*S.config.Ts;
    S.algorithms.RL.vf      = max(min(S.algorithms.RL.vf, S.config.Uconstraints(2,2)), S.config.Uconstraints(2,1));
    % y-axe legged robot
    R                       = [0 1; -1 0];  % R = [cos(-pi/2) -sin(-pi/2); sin(-pi/2) cos(-pi/2)];
    vy                      = R*vx;
    ref_y                   = norm(dot(v,vy)*vy)*sign(dot(v,vy));
    S.algorithms.RL.ey      = ref_y;
    S.algorithms.RL.eyInt   = S.algorithms.RL.eyInt + S.algorithms.RL.ey * S.config.Ts;
    state_y                 = [ref_y; S.algorithms.RL.vl; S.algorithms.RL.ey; S.algorithms.RL.eyInt];
    action_y                = getAction(S.algorithms.RL.alg.agent, state_y);
    S.algorithms.RL.vl      = S.algorithms.RL.vl + action_y{1}*S.config.Ts;
    S.algorithms.RL.vl      = max(min(S.algorithms.RL.vl, S.config.Uconstraints(3,2)), S.config.Uconstraints(3,1));
    %
    S.exec_time.t_rl1dim    = [S.exec_time.t_rl1dim, toc];
    S.simulation_input.u    = [0; S.algorithms.RL.vf; S.algorithms.RL.vl; 0];
    S.data.Ctrls            = [S.data.Ctrls, S.simulation_input.u];
end
...
function S = init_RL1dim(S)
    path = '/home/nahuel/Documents/Reinforcement Learning/Matlab implementations/';
    load(strcat(path, 'trainedQuadrupedAgent_good1.mat'), 'agent');
    S.algorithms.RL.alg.agent   = agent;
    %
    S.algorithms.RL.af          = 0;
    S.algorithms.RL.al          = 0;
    S.algorithms.RL.aw0         = 0;
    %
    S.algorithms.RL.vf          = 0;
    S.algorithms.RL.vl          = 0;
    S.algorithms.RL.w0          = 0;
    %
    S.algorithms.RL.ex          = 0;
    S.algorithms.RL.exInt       = 0;
    S.algorithms.RL.ey          = 0;
    S.algorithms.RL.eyInt       = 0;
...
function S = gen_x0(S)
    ...
    z_pos               = 0.29;
    %    
    S.init_condition.x0       = [ xy_0; z_pos; theta0 ] + [0.15*randn; 0.15*randn; 0; 0]; % + [0.5*randn; 0.5*randn; 0; randn];
    %
