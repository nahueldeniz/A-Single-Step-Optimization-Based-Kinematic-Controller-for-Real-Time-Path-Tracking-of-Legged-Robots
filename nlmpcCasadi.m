classdef nlmpcCasadi < handle
    ...
    methods
        function obj = nlmpcCasadi(Nc,F,mtxs,dims,constraints,Ts)
            obj.Nc          = Nc;            
            obj.nx          = dims.nq;
            ...
            %                                                
            optParam        = {X0, Xref,vec(Q),vec(Qf),vec(R),U_last};
            ...
            optVar          = {Xk{:},Uk{:}};
            if obj.Nc==1
                obj.optVar_lb   = [ obj.xf_lb; obj.u_lb];
                obj.optVar_ub   = [ obj.xf_ub; obj.u_ub];
            else
                obj.optVar_lb   = [ repmat(obj.x_lb,obj.Nc-1,1); obj.xf_lb; repmat(obj.u_lb,obj.Nc,1)];
                obj.optVar_ub   = [ repmat(obj.x_ub,obj.Nc-1,1); obj.xf_ub; repmat(obj.u_ub,obj.Nc,1)];
            end
            %
            state_constraints       = {};
            ...
            for i=1:obj.Nc
                ...
                % Compute cost --------------------------------------------
                if i==obj.Nc
                    J  = J + mtimes(mtimes((Xk{i}-Xref)',Qf),(Xk{i}-Xref)) + mtimes(mtimes((Uk{i})',R),(Uk{i}));
                else
                    J  = J + mtimes(mtimes((Xk{i}-Xref)',Q),(Xk{i}-Xref)) + mtimes(mtimes((Uk{i})',R),(Uk{i}));
                end
            end                
            % All states constraints, including the multiple shooting _____
            nlp_constraints         = {state_constraints{:} deltaU_constraints{:}};
            obj.nlp_constraints_lb  = [state_constraints_lb(:); deltaU_constraints_lb(:)];
            obj.nlp_constraints_ub  = [state_constraints_ub(:); deltaU_constraints_ub(:)];            
...
%             obj.solver                          = casadi.qpsol('solver', 'qpoases', problem, qpoptions);
        ...
        function solve(obj)
            ...
            obj.current_parameters = vertcat(obj.x0bar, obj.xref, vec(obj.Q), vec(obj.Qf), vec(obj.R), obj.u_last);
            ...
            obj.Utraj   = reshape(x_opt(obj.Nc*obj.nx+1:end),obj.nu,obj.Nc);
            %
            obj.u_k     = obj.Utraj(:,1);
            obj.u_last  = obj.u_k;
        end                        
